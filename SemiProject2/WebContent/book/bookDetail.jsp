...
<div class="container-700">
	...
	<div class="row text-left">
		<h1><%=bookDto.getBookTitle()%></h1>
		<br>
		<div>
			<span><%=bookDto.getBookAuthor()%> (지은이)&nbsp;&nbsp;</span> <span><%=bookDto.getBookPublisher()%>&nbsp;&nbsp;</span>
			<span><%=bookDto.getBookPubDate()%></span> <span
				class="container-right"><a
				href="<%=root%>/book/bookEdit.jsp?no=<%=bookDto.getBookNo()%>">수정</a></span>
			<span class="container-right"><a
				href="bookDelete.kh?bookNo=<%=bookDto.getBookNo()%>">삭제</a></span>
 ...
 %> <img title="<%=bookList3.get(i).getBookTitle()%>"
	...
	<div class="row text-left book-detail-semi-box2">
		...
		%><div class="review-regit-reply">
			...
			<div style="display:inline-block" class="review-row">
				<form action="<%=root%>/review/reviewInsert.kh" class="review_input_form" method="post">
					...
					<textarea name="review_content" placeholder="리뷰를 입력하세요" class="review_inbox_text"></textarea>

					<input  class="rate_star" type="hidden" name="review_rate"value="1"> 
					<input type="hidden" name="review_book" value="<%=no%>"> 
					<input type="hidden" name="review_member" value="<%=member%>"> 
					
					</form>
			</div> 
			<div  style="display:inline-block;width:20%;vertical-align: top;margin-top: 45px;margin-left: 25px;"  >
				<input id="review_insert_button" type="button" class="form-btn form-btn-positive btn" value="등록" style="margin-left: 25;height:57px;font-size:20px;width: 95px;margin-top: 7px;">
			</div>
		</div>
		<%
		}
			...
			<div class="row">
				...
				<div class="text-right" style="display: inline-block; width: 48%;">
					<div style="display: inline-block;">
						<input class="edit_button form-btn form-btn-positive btn" type="button"
					...
					<div style="display: inline-block;">
						<form action="<%=root%>/review/reviewDelete.kh">
							...
							<input class="form-btn form-btn-positive btn" type="submit" value="삭제" style="display: inline-block;background-color:#ff6b6b;">
						</form>
					</div>
				</div>
				<%
		...
		<div class="afterEdit hidden" id="afterEdit<%=bookReviewDto.getReviewNo()%>">
			<div style="display:inline-block" class="review-row">
				<form class="review_edit_form" action="<%=root%>/review/reviewEdit.kh" method="post">
				...
				<textarea class="review_inbox_text" placeholder="<%=bookReviewDto.getReviewContent()%>"
			...
			<div  style="display:inline-block;width:20%;vertical-align: top;margin-top: 45px;margin-left: 25px;"  >
				   	<input id="review-edit-button" class="form-btn form-btn-positive btn" type="submit" value="수정" style="display:inline-block;width:45%;">
				 	<input class="cancle_button form-btn form-btn-positive btn" type="button" data-reviewno="<%=bookReviewDto.getReviewNo()%>"value="취소" style="disply:inline-block;width:45%;background-color:#ff6b6b;">
					</div>
