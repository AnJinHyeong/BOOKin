 	...
 	<form action="purchaseInsert.kh"  method="post">
			...
			<table>
				...
				<tbody>
					<%for(int i=0;i<bookList.size();i++){ %>	
						<tr>	
							...
							<td style="text-align:right;">
								<input class="purchaseAmount" name="purchaseAmount" type="number" min="1" value=<%=amounts[i] %> style="width:40px; margin-left:60px;"/>
							</td>    			
							<td style="text-align:center"><%=bookList.get(i).getBookDiscount() %></td>
							<td>
								<button class="btn-del purchase-ok purchase-link-btn" type="button" style="width: 21px; color: #FF3232;" id="<%=bookList.get(i).getBookNo()%>" >X</button>
							</td>
						</tr>
					<%} %>
					...
					<tr>
						...
						<td colspan='5' class="table-result">
							<span>총상품가격 : </span>
							<span class="sum_price"><%=sum_price-2500 %></span>
							<span>원 + 배송비 : 2500 = 합계 : </span>
							<span class="table-total-price sum_price"><%=sum_price %></span>
							<span style="color:#ff6b6b;"> 원</span> 
						</td>
		     		</tr>
		     	</tbody>
			</table>
		...
		<div class="row text-left book-detail-semi-box"> 
			...
			<div>
				...
				<div><%=memberDto.getMemberName() %></div>
				<br>
				<div>
