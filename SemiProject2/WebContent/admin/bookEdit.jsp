...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
    	...
    	BookDto bookDto = bookDao.get(Integer.parseInt(request.getParameter("bookNo")));
    	GenreDto curGen=genreDao.get(bookDto.getBookGenreNo());
     	GenreDto parentGenre=genreDao.get(curGen.getGenreParents());
     	GenreDto topgenre =null;
     	if(curGen.getGenreParents()!=null){
	    	topgenre = genreDao.get(parentGenre.getGenreParents());
     	}
	%>
...
<jsp:include page="/template/adminSidebar.jsp"></jsp:include>
	<section>
		...
		<form action="<%=root%>/book/bookEdit.kh" method="post" enctype="multipart/form-data">
		<div class="admin-content_area">
			<div class="admin-content">
				<div class="admin-content_title">
					카테고리
				...
				<div class="show_genre" style="visibility : visible;">
				<%if(topgenre!=null){ %>
				<span class="show_genre1"><%=topgenre.getGenreName()%> > </span>
				<%}else{ %>
				<span class="show_genre1"></span>
				<%} %>
				<span class="show_genre2"><%=parentGenre.getGenreName()%> > </span>
				<span class="show_genre3"><%=curGen.getGenreName()%></span>
				</div>
			</div>
			<input type="hidden" name='bookGenre' class="input_genre" value='<%=curGen.getGenreName()%>'/>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content">
				...
				<input type='text' name="bookTitle" required value="<%=bookDto.getBookTitle()%>">
				</div>
			</div>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content ">
				...
				<div class="content_price">
					<span>정가</span>
					<input required type="number" placeholder="숫자만 입력" name="bookPrice" value="<%=bookDto.getBookPrice()%>">
					<span>원</span>
				</div>
				<div class="content_price">
					<span>판매가</span>
					<input type="number" placeholder="숫자만 입력" name="bookDiscount" value="<%=bookDto.getBookDiscount()%>">
					<span>원</span>
				</div>
			</div>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content ">
				...
				<label for="input-file">
					...
					<img title="<%=bookDto.getBookTitle() %>" class="admin-upload_img" src="<%=bookDto.getBookImage()%>">
					<%}else{ %>
					<img title="<%=bookDto.getBookTitle() %>" class="admin-upload_img" src="<%=root%>/book/bookImage.kh?bookNo=<%=bookDto.getBookNo()%>">
					<%} %>
				</label>
				<input class="input_img" type="file" accept=".png, .jpg, .gif" id="input-file" name="bookImage" style="display: none"/>
			</div>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content">
				...
				<div class="admin-input_text">
				<textarea name="bookDescription"><%=bookDto.getBookDescription() %></textarea>
				</div>
			</div>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content">
				...
				<div class="admin-input_text">
				<input type='text' name="bookPublisher" value="<%=bookDto.getBookPublisher()%>">
				</div>
			</div>
		</div>
		...
		<div class="admin-content_area">
			<div class="admin-content">
				...
				<div class="admin-input_text">
				<input type="date" name="bookPubDate" value="<%=bookDto.getBookPubDate()%>">
				</div>
			</div>
		</div>
